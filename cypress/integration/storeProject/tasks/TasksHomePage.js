...
class TasksHomePage {
  ...
  validateOverviewCards = () => {
    return this.homepage.overviewcards.getOverviewCards().each(($el, index) => {
      ...
      expect($el.children("div").children("h4").children("a").text()).to.be.eq(
        prods.Items[index].title
      );
      expect($el.children("div").children("h5").text()).to.be.eq(
        "$" + prods.Items[index].price
      );
  ...
  selectModalLink = (li) => {
    return this.homepage.header.getNavBarLis().each(($e, i) => {
      cy.log($e.text())
